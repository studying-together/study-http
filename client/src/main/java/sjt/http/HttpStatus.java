...
/**
 * org.springframework.http.HttpStatus  @since 3.0를 수정해서 만들었습니다.
 * 상태코드를 Series 별로 나눠둔 부분은 제거했습니다.(ex: 100대 200대 상태코드 등)
 */
public enum HttpStatus {
    ...
    public static HttpStatus valueOf(final int statusCode) {
        return resolve(statusCode).orElseThrow(() -> new IllegalArgumentException("No matching constant for [" + statusCode + "]"));
    }

    public static Optional<HttpStatus> resolve(final int statusCode) {
